...
(function () {
  ...
  function withTimeout(promise, ms, label) {
    ...
    const timeout = new Promise(resolve => {
      timer = setTimeout(() => {
        resolve({ data: null, error: { message: `${label} не ответил за ${ms} мс` } });
      }, ms);
    });
  ...
  async function fetchApprovedSubmissions(force = false) {
    ...
    // Публичные страницы не должны висеть пустыми, пока Supabase долго отвечает.
    // Локальные JSON уже есть на сайте, а одобренные заявки догружаются только если API ответил быстро.
    const query = supabaseClient
      .from("submissions")
      .select("id,type,title,payload,status,created_at,author_name,author_email")
      .eq("status", "approved")
      .order("created_at", { ascending: false });

    const { data, error } = await withTimeout(query, 2500, "Supabase");

    if (error) {
      console.warn("Не удалось быстро загрузить одобренные материалы из Supabase:", error.message);
      approvedCache = [];
      return approvedCache;
    }
  ...
  async function updateSubmission(id, changes) {
    ...
    const loggedEmail = userData?.user?.email || "<нет активной сессии>";

    // Админские изменения делаем через RPC-функцию public.admin_update_submission.
    // Это надёжнее прямого UPDATE через RLS: функция сама проверяет email вошедшего пользователя
    // и возвращает понятную ошибку, если в Supabase указан другой админ.
    const args = {
      submission_id: id,
      new_status: Object.prototype.hasOwnProperty.call(changes, "status") ? changes.status : null,
      new_title: Object.prototype.hasOwnProperty.call(changes, "title") ? changes.title : null,
      new_payload: Object.prototype.hasOwnProperty.call(changes, "payload") ? changes.payload : null,
      new_moderator_note: Object.prototype.hasOwnProperty.call(changes, "moderator_note") ? changes.moderator_note : null
    };

    const { data, error } = await supabaseClient.rpc("admin_update_submission", args);

    if (error) {
      const message = error.message || String(error);
      if (/admin_update_submission/i.test(message) && /not found|schema cache|could not find/i.test(message)) {
        throw new Error("В Supabase ещё не создана функция admin_update_submission. Запусти docs/supabase-fix-admin-actions.sql в SQL Editor и замени YOUR_ADMIN_EMAIL на свой email.");
      }
      throw new Error(`${message} (вход выполнен как: ${loggedEmail})`);
    }

    if (!data) {
      throw new Error(`Supabase не вернул обновлённую строку. Вход выполнен как: ${loggedEmail}. Проверь функцию admin_update_submission в SQL Editor.`);
    }
  ...
  async function deleteSubmission(id) {
    ...
    const { data, error } = await supabaseClient.rpc("admin_delete_submission", {
      submission_id: id
    });

    if (error) {
      const message = error.message || String(error);
      if (/admin_delete_submission/i.test(message) && /not found|schema cache|could not find/i.test(message)) {
        throw new Error("В Supabase ещё не создана функция admin_delete_submission. Запусти docs/supabase-fix-admin-actions.sql в SQL Editor и замени YOUR_ADMIN_EMAIL на свой email.");
      }
      throw new Error(`${message} (вход выполнен как: ${loggedEmail})`);
    }
